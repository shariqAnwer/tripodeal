...
const FlightBooking = () => {
  ...
  const handleConfirm = (e) => {
    if (
      ...
      setErrorFlag(false);
      history.push("/confirmation", { fName, lName, email, mobile });
    } else {
      setErrorFlag(true);
    }
  };
  ...
  return (
    <Grid container>
      <Grid item xs={12} className={classes.filterContainer}>
        <h1>
          Booking Confirmation for Flight: {bookingData?.result?.airlineName}
        </h1>
        <Typography variant="h6">
          {`From: (${bookingData?.result?.deptCity})
           To: (${bookingData?.result?.arivalCity}),
            Flight Number (${bookingData?.result?.flightNbr})`}
        </Typography>
      </Grid>
